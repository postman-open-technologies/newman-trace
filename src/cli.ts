...
async function run(argv): Promise<void> {
  ...
  program
    ...
    .action(async (collection, command) => {
      ...
      try {
        const systemProxyURL = new URL(
          process.env.http_proxy || process.env.HTTP_PROXY // TODO: Take option from command line
        );
        const hasRunError = await execute(options, { systemProxyURL });
        hasRunError && !_.get(options, "suppressExitCode") && process.exit(1);
      } catch (err) {
        console.error(`error: ${err.message || err}\n`);
        err.friendly && console.error(`  ${err.friendly}\n`);
        !_.get(options, "suppressExitCode") && process.exit(1);
      }
    });
  program.addHelpText(
    "after",
...
To get available options for a command:
   ...
   */
  function runNewman(argv, callback) {
    waterfall(
      [
        (next) => {
          // cache original argv, required to parse nested options later.
          (program as any)._originalArgs = argv;
