...
export default {
  cast: {
    ...
    colorOptions: (value) => {
      if (!/^(auto|on|off)$/.test(value)) {
        throw new Error(
          `invalid value \`${value}\` for --color. Expected: auto|on|off`
        );
      }
  ...
  parseNestedOptions: (argv, optionPrefix, options) => {
    const args = [];
    const parsed = { _generic: {} };
    let name, path, len, eqIndex, i;
    ...
    for (i = 0, len = args.length; i < len; i++) {
      ...
      _.set(
        parsed,
        path,
        !args[i + 1] || _.startsWith(args[i + 1], "-") ? true : args[++i]
      );
    }
